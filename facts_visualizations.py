...
def single_trial_plots(condition, trial, a_record, a_tilde_record, formant_record, perturb_record, x_record,argv, formants_produced_record_alltrials=None):
    ...
    plt.plot(formant_record[trial,:,0], 'green' ,label='Formant Record')
    plt.gca().set_prop_cycle(None) 
    #plt.plot(perturb_record[trial,:,0], 'k' ,label='Perturbation')
    plt.gca().set_prop_cycle(None) 
    nan_buffer = sum(np.isnan(formant_record[trial,:,0]))
    #pdb.set_trace()
    produced_buffer_x_axis = np.arange(len(formants_produced_record_alltrials[trial,:,0]))+nan_buffer
    plt.plot(produced_buffer_x_axis,formants_produced_record_alltrials[trial,:,0], 'r' ,label='Formant Produced')
    # plt.legend(['F1'])
    plt.xlabel('frame #')
    plt.ylabel('freq (Hz)')
    plt.legend()
