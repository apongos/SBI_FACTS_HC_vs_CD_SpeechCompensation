...
def transformedDevandCov(Y, y, Wc, R):
    Y1 = (Y.T - y).T
    P=np.matmul(np.matmul(Y1,np.diag(Wc)),Y1.T)+R # (Y1*Wc)#Y1.T
...
def SomatosensoryPrediction(feedbackType,LWPRsom,org_Y,org_y,X1,Wm):
    ...
    for k in range(L):
        for j in range(gv.a_dim*2):
            pred = LWPRsom[j].predict(np.array([X1[j,k]]))
            Y[j,k] = np.asarray(pred).ravel()[0]  # extract scalar (predict may return array)
...
def StateCorrectionForDelay(X2, Wc, Y1, P, z, y, cc_discount_from_delay):
    ...
    #print("sensory",y[0:3])
    #print(f"K: {K}    z-y = {z-y}")
    return np.matmul(K,z-y), np.matmul(K,P12.T), K  #Kalman*prediction error (Eq 5 and/or 6) and covariance update
