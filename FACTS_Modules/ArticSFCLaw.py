...
class ArticSFCLaw_LWPR_noupdate_with_noise(ArticSFCLaw_LWPR_noupdate):
    def __init__(self,artic_configs):
        self.model = LWPR(artic_configs['model_path'])
        self.noise_scale = float(artic_configs['noise_scale'])
        self.adotdot_mean = string2dtype_array(artic_configs['adotdot_mean'], dtype='float') #Get empirical data for this
    ...
    def run(self, xdotdot,a_tilde,ART,i_frm,PROMACT,ms_frm):
            ...
            try:
                ...
                #pdb.set_trace()
                # print(f'adotdot before {adotdot}')
                # print(f'self.noise_scale {self.noise_scale}')
                noise_scale = self.noise_scale
                adotdot_norm = self.adotdot_mean
                # pdb.set_trace()
                a_dotdot_noise = 1e0 * noise_scale * np.random.normal(0,1,adotdot.shape) * adotdot_norm
                a_dotdot_plus_noise = adotdot.copy()
                a_dotdot_plus_noise += a_dotdot_noise
                # print(f'adotdot after {a_dotdot_plus_noise}')

                #print("online: ",np.matmul(IPJAC,xdotdot))
                return a_dotdot_plus_noise
